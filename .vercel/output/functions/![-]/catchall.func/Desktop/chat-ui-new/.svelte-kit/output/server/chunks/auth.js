...
async function getCoupledCookieHash(cookie) {
  ...
  const cookieValue = cookie.type === "elysia" ? cookie.value[config.COUPLE_SESSION_WITH_COOKIE_NAME]?.value : cookie.value.get(config.COUPLE_SESSION_WITH_COOKIE_NAME);
  if (!cookieValue) {
    return "no-cookie";
  }
...
async function authenticateRequest(headers, cookie, url, isApi) {
  const token = cookie.type === "elysia" ? cookie.value[config.COOKIE_NAME].value : cookie.value.get(config.COOKIE_NAME);
  let email = null;
  ...
  if (isApi) {
    const authorization = headers.type === "elysia" ? headers.value["Authorization"] : headers.value.get("Authorization");
