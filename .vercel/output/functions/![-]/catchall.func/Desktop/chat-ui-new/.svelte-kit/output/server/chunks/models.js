...
async function endpointOai(input) {
  ...
  try {
    ...
    console.log(
      {
        baseURL,
        apiKeySource: config.OPENAI_API_KEY ? "OPENAI_API_KEY" : config.HF_TOKEN ? "HF_TOKEN" : "none",
  ...
  const shouldUseUserTokenAuth = (locals) => config.USE_USER_TOKEN === "true" && Boolean(locals?.token) && Boolean(config.OPENAI_BASE_URL) && baseURL.replace(/\/$/, "") === config.OPENAI_BASE_URL.replace(/\/$/, "");
    ...
    return async ({
      ...
      const openAICompletion = await openai.completions.create(body, {
        ...
        headers: {
          ...
          ...shouldUseUserTokenAuth(locals) && locals?.token ? { Authorization: `Bearer ${locals.token}` } : {},
...
const buildModels = async () => {
  ...
  try {
    ...
    if (googleApiKey) {
      ...
      const toGoogleDisplayName = (id) => {
        const cleaned = id.replace(/^models\//, "").replace(/[:]/g, " ").replace(/_/g, "-");
      ...
      try {
        ...
        const googleModels = (parsedGoogle.data ?? []).filter((m) => typeof m.id === "string" && m.id.toLowerCase().includes("gemini")).filter((m) => !m.id.toLowerCase().includes("image")).sort((a, b) => (b.created ?? 0) - (a.created ?? 0));
        const existingIds = new Set(modelsRaw.map((m) => m.id ?? m.name));
        const googleModelsRaw = googleModels.filter((m) => !existingIds.has(m.id)).map(
          (m) => ({
            id: m.id,
            name: m.id,
    ...
    const fallbackRaw = {
      ...
      description: "The model provider could not be reached during build. Please check your API keys and restart.",
      websiteUrl: "https://huggingface.co",
      preprompt: "",
      chatPromptTemplate: void 0,
      promptExamples: [],
      endpoints: [
        {
          type: "openai",
